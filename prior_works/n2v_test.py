...
def train(tr,va,te,y,num_nodes, save_model=False):
    ...
    def evaluate(model,z,ei,y):
        MB = 100_000
        preds = []
        for chunk in ei.split(MB, dim=1):
            preds.append(model.predict(z,chunk).cpu())
    ...
    if save_model:
        torch.save(n2v.forward().detach(), 'n2v.py')
        exit()
    ...
    while True:
        ...
        MBS = 100_000
        batches = tr.split(MBS, dim=1)
        idx = torch.randperm(len(batches))
        for i in range(idx.size(0)):
            loss = anom(z,batches[idx[i]])
            print(f'[{ea}-{i}] {loss}')
            loss.backward()
            anom_opt.step()
...
def get_embs():
    ei,y,nodes = preprocess('unsw')
    train(*ei, y,nodes, save_model=True)


if __name__ == '__main__':
    #for dataset in ['lanl14argus', 'optc', 'unsw', 'optc-argus']:
    for dataset in ['lanl14argus-ts']:
